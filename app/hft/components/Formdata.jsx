...
const Formdata = () => {
    ...
    const handleFormSubmit = async (e) => {
        ...
        try {
            ...
            formdata.append("post", "Horsefires.com/HFT");
            formdata.append("ip", check ? check : "");
            formdata.append("checkout_toogle", checkoutDetail.checkoutChecked);
            formdata.append("blog_url", process.env.NEXT_APP_URL_HFT ?? "horsefires.com/hft");
            ...
            try {
                setLoading(true)
                const form = await fetch(`https://horsefires.com/submitdata.php`, { body: formdata, method: "POST" })
                        ...
                        {CustomerDataValue?.map((data, key) => {
                            return <div className="flex flex-col" key={key}>
                                ...
                                <div className="relative">
                                    <input {...data.inputValue} className={`fontArya py-3 px-2 border w-full rounded-lg focus:outline-none focus:ring-2 focus:ring-red-500 ${data.icon ? 'pl-[3.5rem]' : ''}`} onChange={handleChange} value={CustomerData[data.inputValue.name]} />
